...
> **وضعیت — انجام‌شده:**
> اولویت یک کامل (شیشه‌ی کارت‌ها، `will-change`، پرده‌ی لودینگ، توقف انیمیشن‌ها؛
> زیرمجموعه‌ی فونت بررسی شد و نیازی نبود)، `og:image`، محافظت کدهای مرورگرمحور
> به‌همراه `npm run ssr-check`، و رفع دو باگ لینک شکسته.
>
> **باقی‌مانده:** پیش‌رندر ایستا (SSG) و زنجیره‌ی انتظار صفحات محصول. برای SSG
> اول باید تکلیف `Reveal` روشن شود — محتوای پیش‌رندرشده تا اجرای JS نامرئی
> می‌ماند. راه پیشنهادی: انیمیشن ظاهر شدن با `animation-timeline: view()` به‌جای
> IntersectionObserver، هم‌خانواده با `.scroll-progress` که از قبل در پروژه هست.
