...
| بررسی | نتیجه |
|---|---|
| لینک داخلی معیوب | ۰ — پس از رفع باگ نوبار (پایین را ببینید) |
...
> **این جدول یک بار غلط بود.** سطر «لینک داخلی معیوب: ۰ از ۱۴۷» ادعای درستی
> نبود: نوبار hrefهای `navLinks` را خام رندر می‌کرد و از `page()` رد نمی‌داد، پس
> روی هر ۱۳۵ صفحه‌ی تنوع، ۲۳ لینک به `product/tagheei.html` می‌رفتند که وجود
> ندارد. روی سایت زنده تأیید شد — `404` با عنوان «صفحه پیدا نشد».
>
> درس: **ممیزی لینک باید آدرس را نسبت به صفحه‌ای که در آن رندر شده حل کند**، نه
> اینکه صرفاً وجود مقصد را از ریشه بررسی کند. یک لینک می‌تواند از صفحه‌ی اول
> درست باشد و از `product/` شکسته.
...
> **همه‌ی موارد این فهرست انجام شده‌اند.** اولویت یک (شیشه‌ی کارت‌ها،
> `will-change`، پرده‌ی لودینگ، توقف انیمیشن‌ها)، `og:image`، رفع دو باگ لینک
> شکسته، و اولویت دو: **پیش‌رندر ایستا** و **زنجیره‌ی انتظار**.
>
> آنچه در این مسیر عوض شد:
>
> - **`Reveal` دیگر جاوااسکریپت ندارد.** با `animation-timeline: view()`
>   بازنویسی شد. محتوا پیش‌فرض دیده می‌شود و انیمیشن فقط رویش اجرا می‌شود —
>   شرط لازم برای پیش‌رندر، چون وگرنه markup آماده تا اجرای JS نامرئی می‌ماند.
> - **پرده‌ی لودینگ حذف شد.** با محتوای داخل HTML، پرده روی صفحه‌ی آماده
>   می‌نشست و دقیقاً همان تأخیری را می‌ساخت که پیش‌رندر برای حذفش انجام شد.
> - **`useMediaQuery` و `useTheme` دیگر مقدار اولیه را همگام نمی‌خوانند**، تا
>   اولین رندر سمت مرورگر با markup پیش‌رندرشده یکی باشد و React آن را دور
>   نریزد.
> - **ورودی‌ها از `createRoot` به `hydrateRoot` رفتند.**
